...
export default function CertificatePage() {
  ...
  const [formData, setFormData] = useState<Partial<Record<CertificateType, CertificateData>>>({
    technical: { name: "", urn: "", eventName: "", eventDate: "" },
    cultural: { name: "", urn: "", eventName: "", eventDate: "" },
    sports: { name: "", urn: "", eventName: "", eventDate: "" },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!data?.name || !data?.urn || !data?.eventName || !data?.eventDate) {
    ...
    setFormData(prev => ({
      ...prev,
      [selectedCertificate]: { name: "", urn: "", eventName: "", eventDate: "" },
    }));
            ...
            <div className="flex flex-col gap-3 mt-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <input
                  ...
                  value={formData[selectedCertificate]?.name}
                ...
                <input
                  ...
                  name="urn"
                  placeholder="URN"
                  value={formData[selectedCertificate]?.urn}
                  onChange={(e) => handleInputChange(e, selectedCertificate)}
                  className="border border-gray-300 p-4 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition shadow-sm"
                  required
                />
        ...
        <div className="w-full max-w-3xl flex flex-col gap-5">
          ...
          {submittedCertificates.map((cert, index) => (
            <div key={index} className="border border-gray-200 p-4 rounded-3xl shadow-md bg-white relative">
              ...
              <p><span className="font-medium">Name:</span> {cert.data.name}</p>
              <p><span className="font-medium">URN:</span> {cert.data.urn}</p>
